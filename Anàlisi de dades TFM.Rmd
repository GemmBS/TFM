...
Veiem que el model millora però ho fa molt poc, per tant, decidim quedar-nos amb el model inicial.
...
```{r}
TRAN_VISIT_no<- TRAN_VISIT2[-c(18,58,140,160,228,303,328,358,375,397,476,527,541,560,575,576),]
```
Tornem a aplicar la regressió lineal

```{r}
model_no<- lm(formula = COUNTt ~ MONTH + YEAR + SUNSHINE + TEMPERATURE +      WIND_SPEED + LC, data = TRAN_VISIT_no)
...
```
Encara hi ha autocorrelació.

Correcció autocorrelació amb Cochrane orcutt:
```{r}
library(orcutt)
model3<- cochrane.orcutt(model_no)
model3
summary(model3)
```
...
```{r}
TRAN_VISIT_no$LC<- as.factor(TRAN_VISIT_no$LC)
head(TRAN_VISIT_no$LC)
library(ggplot2)
ggplot(data = TRAN_VISIT_no, aes(x = LC, y = COUNTt, color = LC)) +
    geom_boxplot() +
    theme_bw()
```
...
```{r}
m2<- lm(COUNTt~LC, data = TRAN_VISIT_no)
summary(m2)
...
```{r}
require(nortest)
by(data = TRAN_VISIT_no,INDICES = TRAN_VISIT_no$LC,FUN = function(x){ lillie.test(x$COUNTt)})
```

```{r}
model<- aov(COUNTt ~ LC, data = TRAN_VISIT_no)
summary(model)
```
...
```{r}
attach(TRAN_VISIT_no)
tapply(COUNTt,LC,mean,data=TRAN_VISIT_no)

```
```{r}
pairwise.t.test(TRAN_VISIT_no$COUNTt,TRAN_VISIT_no$LC, p.adj= 'bonferroni')
```

```{r}
TRAN_COUNT6<- aggregate(COUNT ~ LC, TRAN_VISIT_no[TRAN_VISIT_no$YEAR %in% c(2013,2014,2015,2016),], sum)
